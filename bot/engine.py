...
def run_engine(broker, settings, strategy_name="ema_cross_rsi", strategy_params=None, duration_seconds: int | None = None, symbols: list[str] | None = None) -> None:
    ...
    # Monthly symbol rotation (siropkin pattern) to reduce rate-limit load

    active_symbols = rotate_symbol_list(active_symbols)

    logger.info("After rotation: %d active symbols", len(active_symbols))
    ...
    from bot.risk import KillSwitch, StopLossManager, ExitReason, StopMode

    kill_switch_mgr = KillSwitch(settings.max_daily_loss_pct)

    # Stop-loss / take-profit manager (-inspired)
    _roi_table = {}
    if settings.roi_exit_table:
        try:
            _roi_table = {float(k): int(v) for k, v in json.loads(settings.roi_exit_table).items()}
        except (json.JSONDecodeError, TypeError):
            logger.warning('Invalid ROI_EXIT_TABLE JSON, using empty table')

    stop_manager = StopLossManager(
        stop_mode=StopMode.TRAILING_PERCENT if settings.trailing_stop_enabled else StopMode.FIXED_PERCENT,
        default_fixed_pct=settings.stop_loss_pct,
        default_trail_pct=settings.trailing_stop_pct,
        default_trail_dollar=0.0,
        default_max_hold_minutes=settings.max_hold_minutes,
        roi_tiers=_roi_table,
        multi_tier_peak_drawdown=0.03,
    )
    logger.info('StopLossManager configured: mode=%s trail=%.1f%% sl=%.1f%%',
                stop_manager.stop_mode.value,
                stop_manager.default_trail_pct * 100,
                stop_manager.default_fixed_pct * 100)
    ...
    async def _cycle():
        ...
        for sym, pos in list(internal_pos.items()):
            ...
            try:
                ...
                entry_price = pos.get("entry_price", last_price)
                ...
                # --- Dynamic stop-loss / trailing-stop update ---

                # --- Dynamic stop-loss / trailing-stop evaluation via StopLossManager ---
                mgr_result = stop_manager.update_for_position(trade_key=sym, current_price=last_price)
                if mgr_result:
                    computed_stop = mgr_result.get("current_stop", 0)
                    if computed_stop > 0:
                        pos["stop"] = computed_stop
                        prev = pos.get("_prev_stop", 0)
                        if abs(computed_stop - prev) > 0.01:
                            logger.debug("[%s] Stop moved to %.2f (peak=%.1f%%)", sym, computed_stop, mgr_result.get("peak_profit", 0) * 100)
                            pos["_prev_stop"] = computed_stop

                    # Handle any exit signal from StopLossManager (trailing stop / time limit / ROI tier)
                    exit_sig = mgr_result.get("exit_signal")
                    if exit_sig is not None:
                        pos_qty = pos.get("qty", 0)
                        sig_label = str(exit_sig.value) if hasattr(exit_sig, "value") else str(exit_sig)
                        logger.info("EXIT triggered by StopLossManager: %s qty=%d @ %.2f [%s]",
                                    sym, pos_qty, last_price, sig_label)
                        await broker.submit_order(sym, pos_qty, "SELL")
                        engine_state.append_trade(now_str, sym, "SELL", pos_qty, last_price, sig_label)
                        engine_state.append_signal(now_str, sym, -1, sig_label)
                        alerts.send_fill_alert(sym, "SELL", pos_qty, last_price, sig_label, equity=equity)
                        try:
                            stop_manager.remove_position(sym)
                        except Exception:
                            pass
                ...
                # --- Check static take profit hit (not managed by StopLossManager) ---
...
# Symbol exclusion check (blacklist)
            if not settings.is_symbol_allowed(sym):
                continue
            try:
                ...
                if last_signal == 1 and not has_position:
                    ...
                    # Validate trade amount (siropkin min/max constraints)

                    trade_value = qty * last_close

                    if trade_value < settings.min_buying_amount_usd:

                        logger.debug("Buy skipped: %s value=%.2f below min %.2f", sym, trade_value, settings.min_buying_amount_usd)

                        continue

                    if settings.max_buying_amount_usd and trade_value > settings.max_buying_amount_usd:

                        logger.debug("Buy skipped: %s value=%.2f above max %.2f", sym, trade_value, settings.max_buying_amount_usd)
                    ...
                    # Validate buy amount against guidelines
                    buy_val = qty * last_close
                    min_buy = getattr(settings, 'min_buying_amount_usd', 0)
                    max_buy = getattr(settings, 'max_buying_amount_usd', float('inf'))
                    if min_buy > 0 and buy_val < min_buy:
                        logger.info('[%s] SKIP BUY: %.2f USD below minimum %.2f', strategy_name, buy_val, min_buy)
                        continue
                    if max_buy > 0 and buy_val > max_buy:
                        logger.info('[%s] CAP BUY: %.2f USD above maximum %.2f, scaling down', strategy_name, buy_val, max_buy)
                        qty = int(max_buy / last_close)
                        if qty <= 0:
                            continue
                    ...
                    # Register position with StopLossManager for trailing/timing/ROI management
                    try:

                        trade_key = sym

                        stop_mode_override = StopMode.TRAILING_PERCENT if settings.trailing_stop_enabled else StopMode.FIXED_PERCENT

                        stop_manager.new_position(

                            trade_key=trade_key,

                            entry_price=last_close,

                            mode=stop_mode_override,

                            stop_pct=settings.stop_loss_pct,

                            trail_pct=settings.trailing_stop_pct,

                            max_hold_minutes=settings.max_hold_minutes if settings.max_hold_minutes > 0 else None,

                            roi_tiers={float(k): int(v) for k, v in json.loads(settings.roi_exit_table).items()} if settings.roi_exit_table else None,

                        )

                        logger.debug('[%s] Position tracked by StopLossManager (mode=%s)', trade_key, stop_mode_override.value)

                    except Exception as exc:

                        logger.warning('Failed to register position in StopLossManager: %s', exc)
                    ...
                    # Validate sell amount against guidelines
                    sell_val = qty * last_price if last_price else qty * last_close
                    min_sell = getattr(settings, 'min_selling_amount_usd', 0)
                    max_sell = getattr(settings, 'max_selling_amount_usd', float('inf'))
                    if min_sell > 0 and sell_val < min_sell:
                        logger.info('[%s] SKIP SELL: %.2f USD below minimum %.2f', strategy_name, sell_val, min_sell)
                        internal_pos.pop(_k(sym), None)
                        continue
                    if max_sell > 0 and sell_val > max_sell:
                        logger.info('[%s] CAP SELL: %.2f USD above maximum %.2f, scaling down', strategy_name, sell_val, max_sell)
                        qty = int(min(max_sell, pos['qty'] * last_price if last_price else pos['qty'] * last_close) / last_price) if last_price else qty
                        if qty <= 0:
                            internal_pos.pop(_k(sym), None)
                            continue
...
def rotate_symbol_list(symbols_path: str, symbols_dir: str = r"D:\StockTradingBot\data\symbol_rotations",
                        cycle_days: int = 30) -> list[str]:
    """Monthly cycling of symbol lists.

    Maintains JSON manifest files per month that map months to symbol subsets.
    If a manifest file does not exist for the current month, returns all symbols
    ...
    manifest_dir = Path(symbols_dir)
    manifest_dir.mkdir(parents=True, exist_ok=True)
    manifest_file = manifest_dir / f"{month_key}.json"

    try:
        if manifest_file.exists():
            with open(manifest_file, "r") as mf:
                manifests = json.load(mf)
            if month_key in manifests and manifests[month_key]:
                selected = manifests[month_key]
                logger.info("Symbol rotation: %d symbols selected for %s", len(selected), month_key)
                return selected
    except (json.JSONDecodeError, IOError) as exc:
        logger.warning("Failed to read symbol rotation manifest: %s", exc)

    logger.info("No rotation manifest for %s -- using full symbol list (%d symbols)", month_key, len(symbols_path))
    return list(symbols_path)
